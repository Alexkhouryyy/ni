...
export interface Orb {
  ...
  setBrutalMode(active: boolean): void;
  setHyperMode(active: boolean): void;
...
export function createOrb(canvas: HTMLCanvasElement): Orb {
  ...
  // Detect mobile — reduce particle count and skip connection lines for perf
  const isMobile = /Mobi|Android|iPhone|iPad|iPod/i.test(navigator.userAgent)
    || window.innerWidth < 600;
  const N = isMobile ? 400 : 2000;
  ...
  const MAX_LINES = isMobile ? 0 : 8000;
  const linePos = new Float32Array(Math.max(MAX_LINES, 1) * 6);
  ...
  function animate() {
    ...
    // ── Update lines (skipped on mobile) ──
    if (!isMobile && lineAmount > 0.01) {
    ...
    if (studyModeActive) {
      // Study Mode: deep indigo palette
      if (state === "thinking") { mat.color.lerp(new THREE.Color(0x818cf8), 0.015); lineMat.color.lerp(new THREE.Color(0x818cf8), 0.015); }
      else if (state === "speaking") { mat.color.lerp(new THREE.Color(0x6366f1), 0.015); lineMat.color.lerp(new THREE.Color(0x6366f1), 0.015); }
      else { mat.color.lerp(new THREE.Color(0x4f46e5), 0.015); lineMat.color.lerp(new THREE.Color(0x4f46e5), 0.015); }
    } else if (brutalModeActive) {
      // Brutal Honesty Mode: crimson red palette
      if (state === "thinking") { mat.color.lerp(new THREE.Color(0xf87171), 0.015); lineMat.color.lerp(new THREE.Color(0xf87171), 0.015); }
      else if (state === "speaking") { mat.color.lerp(new THREE.Color(0xef4444), 0.015); lineMat.color.lerp(new THREE.Color(0xef4444), 0.015); }
      else { mat.color.lerp(new THREE.Color(0xdc2626), 0.015); lineMat.color.lerp(new THREE.Color(0xdc2626), 0.015); }
    } else if (hyperModeActive) {
      // Hyper Intelligence Mode: electric gold/amber palette
      if (state === "thinking") { mat.color.lerp(new THREE.Color(0xfcd34d), 0.015); lineMat.color.lerp(new THREE.Color(0xfcd34d), 0.015); }
      else if (state === "speaking") { mat.color.lerp(new THREE.Color(0xfbbf24), 0.015); lineMat.color.lerp(new THREE.Color(0xfbbf24), 0.015); }
      else { mat.color.lerp(new THREE.Color(0xf59e0b), 0.015); lineMat.color.lerp(new THREE.Color(0xf59e0b), 0.015); }
    } else {
      // Normal cyan palette
  ...
  return {
    ...
    setStudyMode(active: boolean) { studyModeActive = active; },
    setBrutalMode(active: boolean) { brutalModeActive = active; },
    setHyperMode(active: boolean) { hyperModeActive = active; },
