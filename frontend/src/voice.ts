...
export function createVoiceInput(
  ...
  onInterimSpeech?: (text: string, confidence: number) => void
): VoiceInput {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  recognition.onresult = (event: any) => {
    for (let i = event.resultIndex; i < event.results.length; i++) {
      ...
      } else if (onInterimSpeech) {
        // Interim result — used by Live Conversation Mode for interrupt detection
        const interim = event.results[i][0].transcript.trim();
        const confidence = (event.results[i][0].confidence as number) ?? 0;
        if (interim) onInterimSpeech(interim, confidence);
      }
    }
  };
